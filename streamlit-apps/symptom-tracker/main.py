...
class SymptomChecker:
    ...
    def call_perplexity_api(self, prompt: str, api_key: str, max_tokens: int = 300) -> str:
        ...
        try:
            ...
            if 'choices' in data and len(data['choices']) > 0:
                ...
                raise Exception("No response content from Perplexity API")
    ...
    def generate_followup_questions(self, symptoms: List[str], api_key: str = None) -> List[str]:
        """Enhanced follow-up question generation using Perplexity API"""
        if not api_key:
            ...
            try:
                ...
                response_text = self.call_perplexity_api(prompt, api_key, max_tokens=500)
                ...
                st.error(f"Error generating questions with Perplexity: {e}")
    ...
    def interpret_followup_answers(self, questions_answers: Dict[str, str], api_key: str = None) -> Dict[str, str]:
        """Enhanced answer interpretation with structured extraction using Perplexity API"""
        ...
        try:
            ...
            response_text = self.call_perplexity_api(prompt, api_key, max_tokens=400)
            ...
            st.error(f"Error interpreting answers with Perplexity: {e}")
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            api_key = st.secrets.get("PERPLEXITY_API_KEY") or None
            if api_key:
                st.success("🤖 AI-Enhanced Analysis Active")
                # st.info("Powered by Perplexity AI")
            else:
