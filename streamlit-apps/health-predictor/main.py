...
def call_perplexity_api(prompt, max_tokens=2000):
    """Call Perplexity Sonar Pro API for health insights"""
    if not ai_available or not perplexity_api_key:
...
def generate_comprehensive_health_insights(user_data, diabetes_prob, heart_disease_prob, recommendations):
    """Generate comprehensive health insights with complete lifestyle recommendations using Perplexity Sonar Pro"""
    if not ai_available:
        return "AI insights unavailable - API configuration error"
    
    prompt = f"""
    As a comprehensive medical AI assistant, analyze this health profile and provide detailed personalized insights with complete lifestyle recommendations based on the latest medical research and evidence-based guidelines:
    ...
    **Risk Assessment:**
    ...
    Please provide a comprehensive analysis with the following sections based on current medical research:

    ## 🏥 Health Summary
    Provide a 2-3 sentence overall health assessment and risk overview based on latest medical evidence.

    ## 🎯 Key Risk Factors Identified
    List and explain the most important risk factors based on current medical literature.

    ## 🍽️ Personalized Diet Recommendations
    - **Evidence-based dietary patterns** for diabetes and heart disease prevention
    - **Specific foods to include** with research-backed benefits
    - **Foods to limit or avoid** with scientific explanations
    - **Sample meal plans** based on current nutritional guidelines
    - **Hydration and supplement recommendations** from recent studies

    ## 🏃‍♂️ Exercise & Physical Activity Plan
    - **Current exercise guidelines** from medical organizations
    - **Specific activities** proven effective for diabetes/heart disease prevention
    - **Weekly schedule** based on evidence-based recommendations
    - **Safety considerations** from latest research

    ## 😴 Sleep & Recovery Recommendations
    - **Sleep recommendations** based on current sleep medicine research
    - **Recovery strategies** supported by scientific evidence

    ## 🧘‍♂️ Stress Management & Mental Health
    - **Evidence-based stress reduction techniques**
    - **Mental health strategies** with research backing

    ## 📊 Health Monitoring & Prevention
    - **Monitoring guidelines** from medical organizations
    - **Preventive care schedule** based on current recommendations
    - **Early warning signs** from medical literature

    ## 💡 Daily Health Habits & Routines
    - **Evidence-based daily habits** for optimal health
    - **Lifestyle modifications** proven effective in clinical studies

    ## ⚠️ When to Seek Medical Attention
    - **Medical consultation guidelines** from healthcare organizations
    - **Emergency warning signs** based on medical protocols

    ## 🎯 Personalized Action Plan
    - **Short and long-term goals** based on evidence-based medicine

    Please ensure all recommendations are based on current medical research and include appropriate medical disclaimers. Keep recommendations practical and evidence-based.
    """
    
    return call_perplexity_api(prompt, max_tokens=3000)
...
def generate_enhanced_pdf_report(user_data, diabetes_prob, heart_disease_prob, recommendations, 
    ...
    try:
        ...
        # Title
        title_text = "AI-Enhanced Health Risk Assessment & Evidence-Based Lifestyle Report" if ai_insights else "Health Risk Assessment Report"
        ...
        header_data = [
            ...
            ['Report Type:', 'Comprehensive Health Analysis & Evidence-Based Lifestyle Guide'],
            # ['AI Enhancement:', 'Yes (Perplexity Sonar Pro)' if ai_insights else 'No']
        ]
        ...
        if ai_insights:
            elements.append(Paragraph("Evidence-Based Health Analysis & Lifestyle Recommendations", heading_style))
        ...
        <br/><br/>
        <b>Report Enhanced with:</b><br/>
        • Evidence-based recommendations from latest medical research<br/>
        • Personalized lifestyle modifications based on scientific literature
        """
        ...
        # Personal Health Profile section would follow here
