...
class PerplexityHealthAssistant:
    """Custom class to handle Perplexity Sonar API calls"""
    ...
    def generate_response(self, question, context):
        ...
        # Create the health-focused prompt
        prompt = f"""
        ...
        Please provide a helpful health assistant response:
        """
...
def initialize_chatbot():
    """Initialize the chatbot with Qdrant Cloud and Perplexity"""
    try:
        # Load environment variables
        ...
        perplexity_assistant = PerplexityHealthAssistant(perplexity_api_key)
        
        return retriever, vectorstore, qdrant_client, perplexity_assistant
    
    except Exception as e:
        st.error(f"❌ Error initializing chatbot: {str(e)}")
        st.stop()

def get_response(question, retriever, perplexity_assistant):
    """Get response using retrieved context and Perplexity API"""
    try:
        ...
        response = perplexity_assistant.generate_response(question, context)
...
def main():
    ...
    if prompt := st.chat_input("Ask me about your health concerns..."):
        ...
        with st.spinner("🔍 Searching medical knowledge base..."):
            try:
                retriever, _, _, perplexity_assistant = initialize_chatbot()
                
                # Get response
                response = get_response(prompt, retriever, perplexity_assistant)
